    ...
    <div className="mx-auto max-w-lg space-y-4">
            ...
            <li key={item.id} className="flex items-center justify-between gap-2 text-sm">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500">
                  {unidadesMinimasATexto(item.cantidad, item.producto.factorConversion)}{" "}
                  {item.producto.unidadMedida} × {centavosATexto(item.precioUnitario, simbolo)}
                  {item.descuento > 0 && ` · desc. ${centavosATexto(item.descuento, simbolo)}`}
                </p>
              </div>
              <p className="shrink-0 font-medium text-slate-900">
                {centavosATexto(item.subtotal, simbolo)}
              </p>
            </li>
          ))}
        </ul>
      ...
      {venta.estado === "COMPLETADA" && (
        <div className="rounded-xl bg-white p-4">
          ...
          <AnularVentaForm ventaId={venta.id} />
        </div>
      )}
    </div>
  );
