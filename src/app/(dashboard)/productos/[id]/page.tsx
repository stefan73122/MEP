...
export default async function ProductoDetallePage({
  ...
  const [producto, configuracion, movimientos] = await Promise.all([
    db.producto.findUnique({ where: { id: productoId } }),
  ...
  const lotes = producto.perecedero
    ? await db.lote.findMany({
    ...
    <div className="mx-auto max-w-lg space-y-4">
      ...
      <div className="rounded-xl bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-lg bg-slate-50 p-2">
            ...
            <p className={`font-semibold ${bajoStock ? "text-danger-600" : "text-slate-900"}`}>
              {unidadesMinimasATexto(producto.stockActual, producto.factorConversion)}{" "}
        ...
        <div className="mt-3 flex justify-end">
          <EstadoProductoToggle productoId={producto.id} activo={producto.activo} />
        </div>
      </div>
      ...
      {producto.activo && producto.perecedero && (
        <>
                    ...
                    <li key={lote.id} className="rounded-lg border border-slate-100 p-2 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-slate-900">
                          {unidadesMinimasATexto(lote.cantidad, producto.factorConversion)}{" "}
                          {producto.unidadMedida}
                        </span>
                        <span className="text-xs text-slate-500">
                          Vence {fechaATexto(lote.fechaVencimiento)}
                        </span>
                      </div>
                      {estado && (
          ...
          <div className="rounded-xl bg-white p-4">
            ...
            <EntradaLoteForm productoId={producto.id} unidadMedida={producto.unidadMedida} />
          </div>

          <div className="rounded-xl bg-white p-4">
            <h2 className="mb-3 text-sm font-semibold text-slate-900">Salida o merma</h2>
            <SalidaLoteForm productoId={producto.id} unidadMedida={producto.unidadMedida} />
          </div>
        </>
      )}
      ...
      {producto.activo && !producto.perecedero && (
        <div className="rounded-xl bg-white p-4">
          ...
          <MovimientoForm productoId={producto.id} unidadMedida={producto.unidadMedida} />
        </div>
      )}
            ...
            {movimientos.map((movimiento) => (
              <li key={movimiento.id} className="rounded-lg border border-slate-100 p-2 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">
                    {fechaHoraATexto(movimiento.createdAt)}
                  </span>
                </div>
                ...
                <p className="text-xs text-slate-500">
                  ...
                  {unidadesMinimasATexto(movimiento.stockPosterior, producto.factorConversion)}{" "}
                  {producto.unidadMedida}
                </p>
              </li>
            ))}
