...
// Filtro de fechas compartido por la página de reportes y sus exportaciones a
// CSV: por defecto, el mes en curso hasta hoy.
...
export function obtenerVentasCompletadas(prisma: ClientePrisma, desde: Date, hasta: Date) {
...
export async function obtenerProductosMasVendidos(
  ...
  const items = await prisma.ventaItem.findMany({
    where: { venta: { estado: "COMPLETADA", fecha: { gte: desde, lte: hasta } } },
...
export async function calcularGananciaEstimada(
  prisma: ClientePrisma,
  desde: Date,
  hasta: Date,
): Promise<GananciaEstimada> {
  const items = await prisma.ventaItem.findMany({
    where: { venta: { estado: "COMPLETADA", fecha: { gte: desde, lte: hasta } } },
    include: { producto: true },
  });
...
export function obtenerGastos(prisma: ClientePrisma, desde: Date, hasta: Date) {
...
export async function calcularTotalGastos(prisma: ClientePrisma, desde: Date, hasta: Date): Promise<number> {
  const gastos = await obtenerGastos(prisma, desde, hasta);
...
// Resumen del día en curso: siempre visible en la pantalla de reportes,
// sin necesidad de elegir un rango de fechas (reemplaza el cierre de caja).
export async function obtenerResumenHoy(prisma: ClientePrisma): Promise<ResumenHoy> {
  const ahora = new Date();
  const ventas = await obtenerVentasCompletadas(prisma, inicioDelDia(ahora), finDelDia(ahora));
