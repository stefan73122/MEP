...
export async function previsualizarImportacion(
  ...
  const skusDb = await db.producto.findMany({
    where: { sku: { in: filas.map((f) => f.sku).filter(Boolean) } },
    select: { sku: true },
  });
...
export async function confirmarImportacion(
  ...
  for (const fila of filasValidas) {
    try {
      const yaExiste = await db.producto.findUnique({ where: { sku: fila.sku } });
      if (yaExiste) {
        ...
        if (stockInicial > 0) {
          ...
          if (fila.perecedero && fechaVencimiento) {
            const lote = await tx.lote.create({
              data: { productoId: producto.id, cantidad: stockInicial, fechaVencimiento },
            });
            loteId = lote.id;
          }
