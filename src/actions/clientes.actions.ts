...
export type EstadoFormularioCliente = { error?: string };
...
// campos que el alta normal, pero devuelve el cliente en vez de redirigir
  ...
  try {
    const cliente = await db.$transaction(async (tx) => {
      const nuevo = await tx.cliente.create({
        data: { nombre: nombreLimpio, telefono: telefono.trim() || null, limiteCredito },
      });
...
export async function actualizarCliente(
  ...
  const clienteActual = await db.cliente.findUnique({ where: { id: datos.id } });
  if (!clienteActual) {
    ...
    await db.$transaction(async (tx) => {
      ...
      for (const venta of ventasPendientes) {
        ...
        await tx.pagoVenta.create({
          data: { pagoId: pago.id, ventaId: venta.id, montoAplicado },
        });
