...
export type EstadoVentaForm = { error?: string; requiereConfirmacion?: boolean };
    ...
    ventaId = await db.$transaction(async (tx) => {
      ...
      for (const item of itemsProcesados) {
        ...
        await tx.producto.update({
          ...
          data: { stockActual: item.stockPosterior },
        });
      }
...
export async function anularVenta(
  ...
  if (venta.formaPago === "CREDITO") {
    const pagos = await db.pagoVenta.aggregate({
      where: { ventaId: venta.id },
      _sum: { montoAplicado: true },
    });
    if ((pagos._sum.montoAplicado ?? 0) > 0) {
      return {
        error: "No se puede anular: esta venta ya tiene pagos aplicados.",
      };
    }
  }
      ...
      for (const item of venta.items) {
        ...
        if (producto.perecedero) {
          ...
          const salidas = await tx.movimientoInventario.findMany({
            where: { ventaId: venta.id, productoId: item.productoId, tipo: "SALIDA", loteId: { not: null } },
          });
          for (const salida of salidas) {
            if (!salida.loteId) continue;
            const lote = await tx.lote.findUnique({ where: { id: salida.loteId } });
            if (!lote) continue;
            await tx.lote.update({ where: { id: lote.id }, data: { cantidad: lote.cantidad + salida.cantidad } });
            await tx.movimientoInventario.create({
              data: {
                productoId: item.productoId,
                loteId: lote.id,
