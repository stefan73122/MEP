...
export function GastoForm({ onExito }: { onExito: () => void }) {
    ...
    <form key={formularioId} action={formAction} className="space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input id="montoTexto" name="montoTexto" type="text" inputMode="decimal" placeholder="0.00" required className={clasesInput} />
        </div>
        <div>
      ...
      <button
        type="submit"
        disabled={enviando}
        className="w-full rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white transition hover:brightness-110 disabled:opacity-60"
      >
